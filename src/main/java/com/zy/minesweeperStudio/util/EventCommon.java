...
import com.zy.minesweeperStudio.bean.*;
		...
		for (int i = 1; i < (height + 1); i++) {
			for (int j = 1; j < (width + 1); j++) {
				if (cells[(width + 2) * i + j].what != 9) {
					cells[(width + 2) * i + j].what = (cells[(width + 2) * (i - 1) + j - 1].what == 9 ? 1 : 0)
							+ (cells[(width + 2) * (i - 1) + j].what == 9 ? 1 : 0)
...
//						logger.debug("L: " + misscl + " " + rawEventDetailBean.getEventTime());
			...
			// d
			if (((omstatus == 0) && (lact == -1 ? 1 : 0 + ract == -1 ? 1 : 0) * (olstatus == 1 ? 1 : 0)
					* (orstatus == 1 ? 1 : 0) > 0) || ((omstatus == 1) && (mact == -1))) {
				d++;
				if (tempR == 1) {
					cloneR++;
					r--;
